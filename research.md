...
**작성 일자**: 2026. 06. 02.  
**대상 파일**: `csu_Ethernet.c`, `hal_Ethernet.c`, `hal_DspInit.c`  
...
*   **원인**: `csu_Ethernet.c` 내 `sendEthernetFrame()` 함수에서 패킷 송신을 위해 `s_xTxPktDesc` 구조체 변수를 참조하고 있으나, 이 변수는 `hal_Ethernet.c`에만 `static` 전역 변수로 정의되어 있어 파일 범위를 벗어나 참조가 불가합니다.
...
*   **해결책**: `hal_Ethernet.c` 및 `csu_Ethernet.c`에서 사용된 모든 `dataLength` 필드 참조를 **`validLength`**로 변경합니다.
...
2.  **CM (`hal_Ethernet.c`, `hal_Timer.c`, `hal_IPC.c`)**:
...
1.  **CPU1 측 처리 (`csu_Adc.c` ➡️ `hal_IPC.c`)**:
...
2.  **CM 측 처리 (`hal_IPC.c` ➡️ `csu_Ethernet.c` ➡️ `hal_Ethernet.c`)**:
...
1.  **CM 측 수신 처리 (`hal_Ethernet.c` ➡️ `csu_Ethernet.c`)**:
...
3.  **CPU1 측 제어 반영 (`hal_IPC.c` ➡️ `csu_IPC.c`)**:
...
*   **조치 내용**:
    *   CM 프로젝트의 [hal_Ethernet.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Ethernet.c) 파일에서 암시적 경고를 야기한 `Ethernet_startInterface` 호출은 안전하게 제거함.
...
*   **원인 분석**:
    1. CPU1 코어의 전체 시스템 하드웨어 초기화 소스 코드([hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c))를 교차 정밀 추적함.
...
*   **조치 내용**:
    *   CPU1 프로젝트의 [hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c) 소스 코드 내 이더넷 GPIO 핀 MUX 설정 함수 `initEmacGpioPins()` 내부에 마스터 제어권 위임 API 코드를 안전하게 삽입함.
    *   **추가 코드**:
        ```c
...
*   **원인 분석**:
    ...
    3. CPU1 초기화 코드(`hal_DspInit.c`)를 교차 분석한 결과, 세컨더리 마스터 권한 배분 API인 **`SysCtl_selectSecMaster(SYSCTL_SEC_MASTER_SEL_EMAC, SYSCTL_SEC_MASTER_CM);`** 호출이 완전히 빠져 있음이 발견되었습니다.
    4. 마스터십이 위임되지 않아 CM 코어는 `Initial_Ethernet()` 구동 시 EMAC 레지스터 영역에 접근하지 못했고, 드라이버 내부 검증 단계에서 실패하여 `g_hEMAC`이 NULL로 무효화되었습니다.
*   **조치 제안**:
    *   CPU1 코어의 [hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c) 파일 내 이더넷 GPIO 설정 함수 `initEmacGpioPins()`의 마지막 부분에 EMAC 세컨더리 마스터 위임 API를 추가하여 해결을 도모합니다.
    *   **제안 코드**:
        ```c
...
*   **조치 제안**:
    1. **ePWM8 트리거 주파수를 1kHz (1ms 주기)로 하향**:  
       `hal_Adc.c`에서 `DEFAULT_PWM_HZ` 정의를 `1000u`로 하향 조율하여 CPU 인터럽트 부하를 100배 줄임으로써 통신 인터럽트 등 타 태스크와의 충돌을 원천 예방합니다.
...
*   **원인 분석**:
    ...
    2. **PHY 칩의 상시 셧다운(Shutdown) 방치**:  
       현재 CPU1의 GPIO 설정부([hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c))를 정밀 교차 추적한 결과, 이 GPIO 135번 핀을 출력용으로 설정하고 **`High` (1) 전압을 방출하여 PHY 칩의 강제 하드웨어 리셋을 해제해 주는 필수 초기화 코드가 완전히 빠져 있음**이 발견되었습니다.
...
*   **조치 제안**:
    1. CPU1 프로젝트의 [hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c) 이더넷 GPIO 설정 함수 `initEmacGpioPins` 내부 마지막에 GPIO 135번의 출력 설정 및 리셋 펄스 해제 코드를 추가합니다.
    2. **제안 코드**:
       ```c
...
* **🛠️ 자가진단 및 추적 방법 (디버거 ⏸️ Suspend 활용)**:
  ...
  * 일시 정지 시 노란색 화살표가 멈춰 서서 가리키는 파일과 소스 코드 라인을 육안으로 정밀 확인합니다.
    * **[사례 A] `ipc.h` 의 `IPC_waitForFlag` (또는 `hal_IPC.c` 30라인 `IPC_sync`)에서 멈춘 경우**:
...
* **조치 내용**:
  * CPU1 코어의 [hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c) 소스 코드에서 CM 코어를 기동하는 **`Initial_CmCore();` 호출 위치를 CPU1의 모든 주변장치(ADC, PWM, SPI, SCI 등) 준비가 완벽하게 마쳐진 최우측(동기화 직전) 시점으로 완전히 대이동 교정**하였습니다.
...
- **[MODIFY]** [hal_IPC.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_IPC.c) / [hal_IPC.h](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_IPC.h)
  - `IPC_init()`을 수행하는 조기 초기화 함수 `Initial_IPC_Clear()`를 신설합니다.
  - 기존 `Initial_IPC()`에서는 `IPC_init()` 호출을 제거하고 인터럽트 등록 및 `IPC_sync()` 대기만 수행합니다.

```c
// CPU1 - hal_IPC.c 신설 및 수정안
...
- **[MODIFY]** [hal_IPC.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_IPC.c) (CM 코어)
  - `IPC_init()` 호출을 제거합니다.
...
- **[CPU1 - hal_IPC.c]** (인터럽트 ISR 수신 핸들러 보강)
...
2. **C28x(CPU1) 측의 마스터십(Mastership) 할당 오류**:
   ...
   하지만 CPU1의 [hal_IPC.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_IPC.c) 내 `Initial_IPC_Mastership()`은 다음과 같이 작성되어 있습니다:
   ```c
   // 기존 오류 코드
...
#### [MODIFY] [hal_IPC.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_IPC.c)
...
#### 1) CPU1 - [hal_DspInit.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c) CM 클럭 변경
```c
...
1. **CM 코어의 이더넷 인터페이스 초기화 흐름**:
   CM 코어가 기동되면서 `Initial_Ethernet()` ([hal_Ethernet.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Ethernet.c)) 함수를 호출하여 이더넷 초기화를 수행합니다.
...
### 18.2 해결 방안
- [hal_Ethernet.c](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Ethernet.c) 내에 CM 코어 전역 인터럽트를 활성화/비활성화할 수 있는 래퍼 함수를 구현합니다.
...
**분석 일자**: 2026. 06. 04.  
**대상 파일**: 
- [hal_Timer.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_Timer.c)
- [main.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/main.c)
- [hal_Timer.c (CM)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Timer.c)
...
- **원인 분석**:
  1. **CPU1의 속도 저하 원인 (100us 인터럽트의 블로킹 오버헤드)**:
     - CPU1의 `hal_Timer.c`에서는 100us 주기(`CPUTimer0`) 인터럽트 핸들러 `isr_CpuTimer0` 내부에서 `sendScia_SCI_PC()` 함수를 매번 실행하고 있습니다.
...
- [hal_Ethernet.c (CM)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Ethernet.c)
- [hal_IPC.c (CM)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_IPC.c)
- [hal_IPC.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_IPC.c)
- [hal_DspInit.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_DspInit.c)
- [hal_EpwmTimer.c (CPU1)](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CPU1/HAL/hal_EpwmTimer.c)
...
1. **EINT 실행 시점** (hal_DspInit.c L95): 전역 인터럽트가 활성화되는 순간 EPWM1 ISR(`isr_Epwm1Timer2ms`)이 **즉시 트리거**됩니다. UP-DOWN 카운터가 이미 동작 중이기 때문입니다.

2. **ISR 내 IPC 전송** (hal_EpwmTimer.c L77): ISR에서 `sendEthDataToCM()`→`IPC_sendCommand(IPC_CPU1_L_CM_R, IPC_FLAG1, ...)` 호출.
...
| 항목 | 내용 |
|------|------|
| **현상** | hal_Timer.c에 `CM_CLK_HZ = 125000000U` (AUXPLL 125MHz) |
| **hal_DspInit.c** | `SysCtl_setCMClk(SYSCTL_CMCLKOUT_DIV_1, SYSCTL_SOURCE_AUXPLL)` — 125MHz 설정 |
...
### 20.6 추가 발견: `hal_Ethernet.c`에 중복 `s_xTxPktDesc` 선언

[hal_Ethernet.c:L51](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/HAL/hal_Ethernet.c#L51)에 `static Ethernet_Pkt_Desc s_xTxPktDesc;`가 선언되어 있으나, 실제 TX 전송은 [csu_Ethernet.c:L31](file:///d:/Nexcom/Firmware/01_Project/02_Tester/TMDSCNCD28388D_T/TMDSCNCD28388D_T/TMDSCNCD28388D_T_CM/CSU/csu_Ethernet.c#L31)의 `static Ethernet_Pkt_Desc s_xTxPktDesc;`을 사용합니다. 서로 다른 파일의 `static` 변수이므로 링크 에러는 없지만, `hal_Ethernet.c`의 것은 **사용되지 않는 불필요한 변수**입니다. 제거 대상입니다.
