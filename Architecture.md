...
- **공유 메모리 맵핑 (MSGRAM 폴링 방식)**:
  - `CPU1TOCM MSGRAM` (2KB): CPU1(Writer) ➡️ CM(Reader). 물리 주소 - `0x39000` (CPU1 관점) / `0x20080000` (CM 관점)
  - `CMTOCPU1 MSGRAM` (2KB): CM(Writer) ➡️ CPU1(Reader). 물리 주소 - `0x38000` (CPU1 관점) / `0x20082000` (CM 관점)
  - *하드웨어 제약 및 롤백 내역*: 초기에 GSRAM(GS0/GS1)을 사용하려 했으나, F28388D 하드웨어 설계상 **CM 코어는 GSRAM에 대한 쓰기 권한을 가질 수 없습니다(Hard Fault 유발)**. 따라서 양방향 독립 쓰기가 가능한 전용 **MSGRAM(Message RAM)**으로 구조를 전환하여 Lock-Free 동기화(Seqlock 및 Try-Lock)를 적용하였습니다. MSGRAM은 일반 메모리 맵 방식으로 접근 가능하므로 IPC 인터럽트를 유발하지 않고 폴링이 가능합니다.
...
- **CPU1 ➡️ CM 전송 (CPU1TOCM MSGRAM)**
  ...
  - **데이터 구조체**: `pxDataCpu1ToCm->Payload.TxData` 사용 (파형 `float32_t`, 온도 `float32_t`, 시퀀스 `uint32_t`)
  - **동기화 (Seqlock)**: 100us마다 ADC 및 파형 연산 직후, CPU1이 `seqCount` 카운터를 홀수(쓰기 중)로 만들고 데이터를 쓴 뒤 짝수(쓰기 완료)로 돌려놓습니다. CM은 짝수일 때만 읽으며, 만약 CM이 읽는 도중 값이 변경되면 다시 읽어 데이터 찢김(Tearing)을 완벽히 방지합니다.
- **CM ➡️ CPU1 전송 (CMTOCPU1 MSGRAM)**
  - **주기**: 이더넷 패킷 수신 시 즉각 반영 (최대 10ms~100ms 가변)
  - **데이터 구조체**: `pxDataCmToCpu1->Payload.RxData` 사용 (`seqNum`, `waveType`)
...
- **프로토콜 페이로드 (22 Bytes)** :
  - Header(12B) + Data(8B: Seq, WaveType, DspTemp, WaveVal) + Checksum(2B)
- **구동 아키텍처** :
  - **RX (수신)**: CM 코어 EMAC 하드웨어 인터럽트(`INT_EMAC_RX0`) 기반 처리로 개편하여 폴링 부하를 제거했습니다. 수신된 패킷에서 추출한 제어값(`SeqNum`, `WaveType`)은 CPU1로 즉각 전달하기 위해 **CMTOCPU1 MSGRAM**에 Seqlock 방식으로 안전하게 기록합니다.
  - **TX (송신)**: PC로부터 데이터 요청(Monitoring) 패킷이 수신되었을 때만 응답 패킷(Reflect MSG)을 조립하여 송신하는 구조입니다. CPU1이 지속적으로 갱신 중인 최신 상태값(온도, 파형)을 **CPU1TOCM MSGRAM**에서 Seqlock Spin-Lock으로 찢김(Tearing) 없이 가져옵니다.
...
- **PC 모니터링 시스템 스펙** :
  ...
  - **실시간 렌더링**: `ScottPlot` 라이브러리를 활용하여, 수신된 패킷에서 역직렬화된 파형(Float) 및 온도 데이터를 초고속 2채널 차트로 렌더링합니다.
...
- **CSU (어플리케이션 계층)**: 순수 비즈니스 로직. 레지스터 접근 절대 금지.
  - `csu_Control.c`: 시스템의 심장인 100us 메인 제어 루프. ADC 갱신, 파형 연산 및 MSGRAM을 통한 데이터 교환(Seqlock / Try-Lock 동기화) 총괄.
  - `csu_Adc.c`: 센서 데이터의 물리적 의미 변환(스케일링) 및 필터링.
  - `csu_LED.c`: LED 점멸 알고리즘 및 상태 머신.
  - `csu_SciPc.c`: PC 직렬 통신 프로토콜 패킷 페이로드 조립 및 파싱.
  - `csu_Ipc_cpu1.c`: 통신 데이터 구조체 정의 및 하드웨어 물리 주소(`0x39000`, `0x38000`)에 MSGRAM 포인터(`pxDataCpu1ToCm`, `pxDataCmToCpu1`) 직접 매핑 수행.
  - `CSU_EPWM.c`: EPWM 제어 로직, Duty 및 주파수 알고리즘 처리.
- **HAL (하드웨어 추상화 계층)**: 레지스터, Driverlib 캡슐화. 인터럽트(ISR)와 하드웨어 핀 제어 수행.
  - `hal_DspInit.c`: GPIO Mux, 클럭 설정, CM 코어 부팅 트리거링. (MSGRAM 전환으로 인해 불필요해진 GSRAM 마스터십 초기화 호출 제거됨)
  ...
  - `hal_Ipc_cpu1.c`: IPC 플래그 록업 제어 및 메일박스 인터럽트 플래그 설정. (GSRAM 제어용 `Initial_IPC_Mastership()` 로직 완전 폐기)
...
- **CSU (어플리케이션 계층)**: 
  ...
  - `csu_Ipc_cm.c`: 이더넷 송수신 버퍼 데이터 구조 선언 및 CM 관점 물리 주소(`0x20080000`, `0x20082000`)에 MSGRAM 포인터 직접 맵핑.
- **HAL (하드웨어 추상화 계층)**: 
...
- **완벽한 모터 제어 알고리즘 구현**: 현재 테스트용 파형(Sine, Square, Triangle)을 생성하여 통신하는 구조에서 나아가, 실제 인버터 PWM 스위칭 및 FOC(Field Oriented Control) 등 정밀 모터 제어 루틴을 `csu_Control.c`의 100us 주기에 본격적으로 탑재할 예정입니다.
- **체계 연동 통신망 가입 및 초기화 절차 구현 (Boot Done)**: 전원 인가 및 PBIT 완료 후 화포통제컴퓨터로 `Boot Done` 메시지를 500ms 주기로 송신하고 ACK를 대기하는 부팅 상태머신을 추가할 예정입니다.
- **100ms 주기 상태정보 통신 및 통신 두절 방어 로직**: 망 가입 완료 후 100ms 주기로 상태 메시지를 교환하며, 50회 연속 무응답(5초) 시 통신 두절로 판단하고 소켓을 리셋하는 자동 복구 알고리즘을 개발할 예정입니다.
- **이더넷 송신 패킷 재전송 큐(Queue) 매니저 구현**: 패킷 전송 후 100ms 내 ACK 수신 실패 시, 헤더의 `Send Count` 필드를 조작하여 최대 3회 재전송하는 관리자 로직을 `csu_Ethernet.c`에 추가할 예정입니다.
- **메시지 헤더 포맷 및 Timestamp 규칙 정밀 동기화**: 능동적 메시지 발행 시 `Timestamp` 필드를 `0x0000`으로 할당하는 규약과, ACK 메시지 내부 4바이트 Data 필드 구조를 `Code Info(2B) + Ack Info(2B)`로 완전히 일치시키는 리팩토링을 진행합니다.
- **세부 데이터 페이로드(ICD) 사양 협의 및 반영**: 체계 연동통제안 문서 상 아직 TBD(To Be Determined)로 남겨진 `CBIT`, `IBIT`, `Boot Done`의 구체적인 Data Payload 규격을 확정한 후 `stIpcDataPacket` 구조체에 반영할 계획입니다.
