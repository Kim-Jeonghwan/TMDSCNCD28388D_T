...
## 6. 소프트웨어 아키텍처 (CSU / HAL 계층 구조)

본 프로젝트는 펌웨어 유지보수성과 하드웨어 종속성 탈피를 위해 엄격한 **CSU (Control & Service Unit)** 및 **HAL (Hardware Abstraction Layer)** 3계층 구조를 따릅니다.

### 6.1 CPU1 코어 아키텍처
- **Main (`main_cpu1.c`)**: 시스템 진입점으로 하드웨어 초기화(`DSP_Initialization`) 수행 후, 어플리케이션 제어 인터럽트(`Control_Init`)를 등록하고 CM 코어와 동기화합니다. 유휴 시간에는 라운드 로빈 형태의 주기 태스크(`Cycle_1ms` ~ `Cycle_100ms`)를 관장합니다.
- **CSU (어플리케이션 계층)**: 
  - `csu_Control.c`: 시스템의 심장인 100us 메인 인터럽트(`MainControl_Isr`)를 관장합니다. 여기서 ADC 필터링 로직 호출, 사인파 생성 연산, 그리고 CM 코어로 보낼 IPC 데이터 패킹 등 핵심 비즈니스 로직을 수행합니다.
  - `csu_Adc.c`, `csu_Led.c`: 센서 데이터의 물리적 의미 변환(스케일링)과 LED 점멸 알고리즘 상태 머신을 담당합니다.
- **HAL (하드웨어 추상화 계층)**: 
  - `hal_DspInit.c`: GPIO 핀 MUX 및 권한 양도(Mastership), 클럭 설정, 그리고 CM 코어의 부팅 트리거를 책임집니다.
  - `hal_Epwm.c`, `hal_Adc.c`: MCU의 레지스터를 직접 조작해 타이머 주기, 클럭 분주비, 트리거 소스 등을 설정합니다. 어플리케이션 비즈니스 로직 및 인터럽트 서비스 루틴(ISR)은 철저히 배제됩니다.

### 6.2 CM 코어 아키텍처
- **Main (`main_cm.c`)**: CM 코어 부팅 진입점입니다. `CM_init()` 후 CPU1과의 IPC 동기화 록업을 풀고, 이더넷 PHY 칩이 기상할 때까지 하드웨어 딜레이를 줍니다. 이후 이더넷을 기동하고 CPU1에 Ready 상태를 통보합니다.
- **CSU (어플리케이션 계층)**: 
  - `csu_Ethernet.c`: UDP 통신 로직, 포트 필터링(5001번), MAC 주소 캡처, 그리고 PC 요청에 대한 비동기 응답(Reflect MSG) 패킷 조립을 전담합니다.
  - `csu_Ipc_cm.c`: CPU1으로부터 들어오는 페이로드 구조체(`TxData`)를 파싱하여 이더넷 송신 버퍼용 전역 변수에 안전하게 갱신합니다.
- **HAL (하드웨어 추상화 계층)**: 
  - `hal_Ethernet.c`: ARM Cortex-M4 전용 EMAC 디바이스 드라이버 로직, RX/TX 디스크립터 풀 관리, MDIO 통신을 통한 PHY 레지스터 엑세스 등 물리 계층(MAC/PHY) 제어를 캡슐화합니다.
  - `hal_Ipc_cm.c`: 메시지 램(Message RAM) 인터럽트 셋업 및 IPC 플래그 상태를 제어합니다.
